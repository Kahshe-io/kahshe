...
| `KAHSHE_ADVERTISE_SERVER_MODE` | all | which tables `loadTable` tells to plan server-side: `all`, `indexed` (only a table declaring `kahshe.index`) or `none`. A table not advertised to plans locally and reads exactly the same files — what is given up is the manifest fetch and, for an indexed table, the index pruning. Narrow it when a client cannot do something while a table says planning MUST be server-side: DuckDB cannot run the first `DELETE` or `UPDATE` against such a table, because it disables server planning below a write ([duckdb-iceberg#1223](https://github.com/duckdb/duckdb-iceberg/issues/1223)) and the server-only mode turns that fallback into an error. Once a table carries delete files kahshe stops advertising it anyway, so the block is on clean tables only |
